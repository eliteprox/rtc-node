...
function updateServerControls() {
  if (!uiRefs?.serverStatus || !uiRefs?.serverToggleBtn) {
    return;
  }
  ...
  if (running) {
    ...
    uiRefs.serverStatus.textContent = `Online on ${host}:${port}`;
    uiRefs.serverToggleBtn.textContent = "Stop API Server";
    uiRefs.serverToggleBtn.className = "ddl-button ddl-button-danger";
    alignLocalServerBase(host, port);
  } else {
    uiRefs.serverStatus.textContent = "Offline";
    uiRefs.serverToggleBtn.textContent = "Start API Server";
    uiRefs.serverToggleBtn.className = "ddl-button ddl-button-secondary";
    lastAlignedLocalBase = null;
  }
  uiRefs.serverToggleBtn.disabled = false;
}
...
async function handleServerToggle() {
  if (!uiRefs?.serverToggleBtn) return;
  uiRefs.serverToggleBtn.disabled = true;
  try {
    if (localServerState?.running) {
      await callRtcControl("stop");
      toast("warn", "DayDream Live", "Local API server stopped");
    } else {
      await callRtcControl("start");
      toast("info", "DayDream Live", "Local API server starting");
    }
    await refreshLocalServerState({ quiet: false });
  } catch (error) {
    toast("error", "DayDream Live", `API server control failed: ${error.message}`);
    uiRefs.serverToggleBtn.disabled = false;
  }
}
...
function updateInfoFields(statusPayload, options = {}) {
  ...
  const historyPayload = options.historyPayload ?? statusPayload ?? displayPayload;
  ...
  appendStatusHistory(descriptor, historyPayload);
...
function appendStatusHistory(descriptor, statusPayload) {
  const entry = {
    ts: new Date().toLocaleTimeString(),
    badge: descriptor.badge,
    remote: descriptor.remoteText,
    http: statusPayload?.remote_status?.http_status ?? "n/a",
    body: statusPayload?.remote_status?.body ?? statusPayload,
    queue: statusPayload?.queue_depth ?? 0,
    buffered: statusPayload?.queue_stats?.buffered ?? 0,
  };
  statusHistory.unshift(entry);
  statusHistory = statusHistory.slice(0, 50);
  if (uiRefs) {
    const logEntries = statusHistory
      .map(
        (item) =>
          `<div class="ddl-log-entry"><strong>[${item.ts}]</strong> ${item.badge} · HTTP ${item.http} · Q ${item.queue}${item.buffered ? ` (buf ${item.buffered})` : ""}<br/>${item.remote || ""}</div>`
      )
      .join("");
    uiRefs.logHistory.innerHTML = logEntries || "<div class='ddl-log-entry'>No history yet.</div>";
    uiRefs.statusJson.textContent = JSON.stringify(entry.body, null, 2);
  }
}
...
function buildPanel(el) {
  ...
  serverCard.className = "ddl-config-card";
  const serverTitle = document.createElement("div");
  serverTitle.className = "ddl-note";
  serverTitle.textContent = "Local API Server";
  const serverStatus = document.createElement("div");
  serverStatus.className = "ddl-label";
  serverStatus.textContent = "Checking...";
  const serverButton = document.createElement("button");
  serverButton.className = "ddl-button ddl-button-secondary";
  serverButton.textContent = "Start API Server";
  serverButton.onclick = () => handleServerToggle();
  serverCard.appendChild(serverTitle);
  serverCard.appendChild(serverStatus);
  serverCard.appendChild(serverButton);
  controlsPanel.appendChild(serverCard);
  ...
  historyBox.className = "ddl-log-history";
  historyBox.innerHTML = "<div class='ddl-log-entry'>No history yet.</div>";

  const jsonViewer = document.createElement("pre");
  jsonViewer.className = "ddl-json-viewer";
  jsonViewer.textContent = "{}";

  logsPanel.appendChild(historyBox);
